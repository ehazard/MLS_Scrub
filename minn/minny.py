...
url = "https://www.mnufc.com/schedule?month=all&year=2017&club_options=11&op=Update&form_build_id=form-BPPi4SqXiex1AAwKG1uOX5w95mfJuhgbIkG9NlKyXxQ&form_id=mp7_schedule_hub_search_filters_form"
...
array = []
dates = []
MC = "https://matchcenter.mnufc.com/matchcenter/"
...
if len(opp) == len(dates):
    ...
    for i in range(0,length):
        if dates[i] == "-05-20-":
            temp = MC + dates[i] + "montreal-impact-vs-portland-timbers/feed"
        else:
            temp = MC + dates[i] + opp[i] + "/feed"
        fURL.append(temp)
else:
...
for i in fURL:
    ...
    for i in allButtons:
        src = i.find('img')['src']
        if src == "https://img.mlsdigital.net/www.mlssoccer.com/7/image/234567/x50.png":
            subN += 1
    subs.append(subN)
